import { Plus } from "lucide-react";
import { useState } from "react";
import { ActivitiesProvider } from "../../context/activities/ActivitiesProvider";
import { LinksProvider } from "../../context/links/links-provider";
import { Activities } from "./activities";
import { CreateActiveModal } from "./create-activity-modal";
import { DestinationAndDateHeader } from "./destination-and-date-header";
import { Guests } from "./guests";
import { ImportantLinks } from "./important-links";

export function TripDetailsPage() {
  const [isCreateActivityModalOpen, setIsCreateActivityModalOpen] = useState(false)

  function openCreateActivityModal() {
    setIsCreateActivityModalOpen(true)
  }

  function closeCreateActivityModal() {
    setIsCreateActivityModalOpen(false)
  }
  
  return (
    <div className="max-w-5xl px-6 py-10 mx-auto space-y-8">
      <DestinationAndDateHeader />
      <ActivitiesProvider>
        <main className="flex gap-16 px-6">
          <div className="flex-1 space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-3xl font-semibold">Atividades</h2>
              <button onClick={openCreateActivityModal} className="bg-lime-300 text-lime-950 rounded-lg px-5 py-2 font-medium flex items-center gap-2 hover:bg-lime-400">
                <Plus className="size-5 text-lime-950" />
                Cadastrar atividade
              </button>
            </div>
            <Activities />
          </div>
          <div className="w-80 space-y-6">
            <LinksProvider>
              <ImportantLinks />
            </LinksProvider>
            <div className="w-full h-px bg-zinc-800" />
            <Guests />
          </div>
        </main>

        {isCreateActivityModalOpen && (
          <CreateActiveModal closeCreateActivityModal={closeCreateActivityModal} />
        )}
        {/* <button onClick={openCreateActivityModal}>Criar Atividade</button> */}
    </ActivitiesProvider>
    </div>
  )
} 